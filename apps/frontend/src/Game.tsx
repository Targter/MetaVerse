...
const Arena = () => {
  ...
  const handleWebSocketMessage = (message: any) => {
    ...
    switch (message.type) {
      case 'space-joined':
        ...
        console.log("message.payload.users:",message.payload.users) 
  ...
  useEffect(() => {
    ...
    if (currentUser && currentUser.x) {
      ...
      ctx.arc(currentUser.x * 50 + 30, currentUser.y * 50 +30, 20, 0, Math.PI * 2);
      ctx.fill();
      ctx.fillStyle = '#6e0d0d';
      ctx.font = '74px Arial';
      ctx.textAlign = 'center';
      ctx.fillText('You', currentUser.x * 50 + 100, currentUser.y * 50 + 100 + 40);
    }
  ...
  return (
    <div className="p-4 bg-black " onKeyDown={handleKeyDown} tabIndex={0}>
        <h1 className="text-2xl font-bold mb-4 bg-black">Arena</h1>
        <div className="mb-4">
          <p className="text-sm text-gray-600">Token: {params.tokenId}</p>
